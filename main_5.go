...
func handleUpdate(bot *tgbotapi.BotAPI, store *Store, update tgbotapi.Update) {
	...
	case "export":
		if !isAdmin(msg.From.ID) {
			bot.Send(tgbotapi.NewMessage(msg.Chat.ID, "Only admins can export the user list."))
			return
		}
		list, err := store.allUsersSorted()
		if err != nil {
		...
		if len(list) == 0 {
			bot.Send(tgbotapi.NewMessage(msg.Chat.ID, "No users tracked yet."))
			return
		}
		csvBytes, err := buildUsersCSV(list)
		if err != nil {
			log.Println("buildUsersCSV error:", err)
			bot.Send(tgbotapi.NewMessage(msg.Chat.ID, "Something went wrong building the export, try again."))
			return
		}
		doc := tgbotapi.NewDocument(msg.Chat.ID, tgbotapi.FileBytes{
			Name:  "giveaway_users.csv",
			Bytes: csvBytes,
		})
		doc.Caption = "📄 " + strconv.Itoa(len(list)) + " tracked users. Opens directly in Excel."
		if _, err := bot.Send(doc); err != nil {
			log.Println("send export error:", err)
			bot.Send(tgbotapi.NewMessage(msg.Chat.ID, "Couldn't send the export file, try again."))
		}
...
func setupCommands(bot *tgbotapi.BotAPI) {
	commands := []tgbotapi.BotCommand{
		...
		{Command: "export", Description: "(Admins) Download all tracked users as a CSV/Excel file"},
...
// buildUsersCSV renders the full user list as CSV bytes, ready to send as
// a Telegram document and open directly in Excel. A UTF-8 BOM is prepended
// so Excel on Windows correctly renders non-Latin names.
func buildUsersCSV(list []*UserStats) ([]byte, error) {
	var buf bytes.Buffer
	buf.Write([]byte{0xEF, 0xBB, 0xBF}) // UTF-8 BOM

	w := csv.NewWriter(&buf)
	_ = w.Write([]string{"telegram_id", "username", "first_name", "contacts_added", "eligible"})

	for _, u := range list {
		eligible := "no"
		if u.ContactsAdded >= 5 {
			eligible = "yes"
		}
		_ = w.Write([]string{
